...
%token TOK_DIR TOK_ROOT

%nonassoc ')'
%right TOK_IF TOK_ELSE TOK_POS TOK_NEG TOK_ORD TOK_CHR
%right '=' '!'
%left  '+' '-' '*' '/' '%' '<' '>'
%left  TOK_CALL TOK_ARRAY TOK_FIELD
%left  TOK_EQ TOK_NE TOK_LE TOK_GE
%left  '[' '.'
%nonassoc TOK_NEW TOK_PAREN
...
ifelse      : TOK_IF '(' expr ')' statement TOK_ELSE statement
            ...
            | TOK_IF '(' expr ')' statement
              { $$=new_astree(NONTERM_IFELSE, $1->filenr, 
                    $1->linenr, $1->offset, "ifelse");
                adopt1($$, $3); adoptgchild($$, $3); adoptgchild($$, $5); }
            ;
